...
create table Member_Enroll_&month. as 
	...
	b.C_D_Coverage,
	b.SNP_Type
from &out..&pfx._StartEnrollTemp where a.PTC_CNTRCT_ID_&month. not in ('') or a.PTD_PBP_ID_&month. not in ('')
left join &input..plan_type b on COALESCE(a.PTC_CNTRCT_ID_&month.,a.PTD_CNTRCT_ID_&month.) = b.contract and COALESCE(a.PTC_PBP_ID_&month.,a.PTD_PBP_ID_&month.)  = b.PBP;
quit;
%mend;
...
data sh054820.&pfx.FFS_All_Claims ;
	set FFS_Claims_: ;
...
proc sql;
	create table &out..&pfx.FFS_All_Claims_2 as
	select 
			a.*
		,b.contract
		,b.plan_type
		...
		,b.C_D_Coverage
		,case when b.month is missing then 0 else 1 end as enroll_find
	from sh054820.&pfx.FFS_All_Claims as a
	left join &out..&pfx._Member_Enroll_HIV as b
	on a.BENE_ID = b.BENE_ID and a.Month = b.Month
	;
quit;
...
proc summary nway missing data= &out..&pfx._ALL_Revenue_HIV ;
	...
	output out= &out..HIV_Revenue_Summary (drop=_type_ rename=(_freq_=Row_Ct))
	sum=;
run;


*Define macro Variables for Claims Fields;
%LET CatVarsCl =  Plan_Type SNP_Type LI B_G Drug ROGARZO_PDE RUKOBIA_PDE TROGARZO_FFS;
...
Proc sql;
create table &out..Member_count as 
select 
	ROGARZO_PDE, RUKOBIA_PDE, TROGARZO_FFS, count(distinct bene_id) as member_count
	from &out..&pfx._memberlist_2020
	group by ROGARZO_PDE, RUKOBIA_PDE, TROGARZO_FFS;
quit;
