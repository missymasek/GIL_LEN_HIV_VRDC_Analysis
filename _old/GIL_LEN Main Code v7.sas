...
*Read in GIL34_plan_type, create format (MM - alternative to hash table or left join);
...
data Member_Enroll_&month. (keep=BENE_ID Month Contract PBP LI Enroll C_D_Coverage SNP_Type);
	...
	*MM - Append needed fields from GIL34_plan_type;
...
*Create format for pulling info from ALL_Revenue_HIV (MM - alternative to hash table or left join);
%macro crtfmt_rev_hiv(tblname, fname, ftype);
data &tblname/view=&tblname;
set &out..&pfx._ALL_Revenue_HIV end=done;
retain fmtname "&fname" hlo ' ' type "&ftype";
/*length start $11;*/
...
*Create format for pulling info from memberlist_2020 (MM - alternative to hash table or left join);
%macro crtfmt_memlist(tblname, fname, ftype);
data &tblname/view=&tblname;
set &out..&pfx._memberlist_2020 end=done;
retain fmtname "&fname" hlo ' ' type "&ftype";
/*length start $11;*/
start = trim(Bene_ID);
label=&tblname;
...
*MM - Append needed fields from ALL_Revenue_HIV;
...
*MM - Append needed fields from memberlist_2020;
...
*MM - Only keep data for HIV members, separate step for troubleshooting purposes;
